...
test("Buyer searches, views listing list, and opens listing detail", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await page
    .locator(".home-search-grid select")
    .nth(0)
    .selectOption("Hồ Chí Minh");
...
test("Owner can open create property page and enter listing content", async ({
  page,
}) => {
  ...
  await expect(page.locator("body")).toContainText(
    /Can ho Playwright moi|mô tả|mo ta/i,
  );
});

test("Admin can open pending approval page and see pending listing", async ({
  page,
}) => {
