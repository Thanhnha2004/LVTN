...
describe('listing-service', () => {
  ...
  test('returns filtered listings with pagination', async () => {
    ...
    expect(pool.query.mock.calls[0][1]).toEqual(expect.arrayContaining(['apartment', 'sale', '%HCM%', 1000000, 5, 0]));
  });

  test('returns listing detail and increments view_count', async () => {
    pool.query
      .mockResolvedValueOnce([[{ id: 1, title: 'Nha pho', images: 'a.jpg,b.jpg' }]])
    ...
    expect(pool.query).toHaveBeenCalledWith('UPDATE properties SET view_count = view_count + 1 WHERE id = ?', ['1']);
  });
...
test('rejects invalid bbox filter', async () => {
  ...
  test('returns similar listings by type transaction city and price range', async () => {
    pool.query
      .mockResolvedValueOnce([[{ type: 'apartment', transaction_type: 'sale', city: 'TP.HCM', price: 3000000000 }]])
    ...
    expect(res.status).toBe(200);
    expect(res.body.data).toHaveLength(1);
    expect(pool.query.mock.calls[1][1]).toEqual(['1', 'apartment', 'sale', '%TP.HCM%', 1500000000, 4500000000]);
  });
